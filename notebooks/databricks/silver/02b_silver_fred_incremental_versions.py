...
# MAGIC %md
# MAGIC # 02b Silver FRED Versioned Incremental Cleaning
# MAGIC
# MAGIC Performs daily incremental Silver cleaning by re-processing only Bronze series with changed observation versions.
# MAGIC
# MAGIC This notebook is designed for the versioned Bronze schema built by
# MAGIC `01b_bronze_fred_incremental_versions.py` after the initial Silver bootstrap has created the target tables.
...
REQUESTED_BRONZE_RUN_ID = dbutils.widgets.get("bronze_run_id").strip() or None
cleaned_table_name = f"{CATALOG}.{SILVER_SCHEMA}.fred_observation_versions_cleaned"
if not table_exists(cleaned_table_name):
    raise ValueError(
        "Silver incremental cleaning requires an existing cleaned Silver table. "
        "Run 02a_silver_fred_bootstrap_versions.py first."
    )

if SERIES_IDS_PARAM.upper() == "ALL":
    series_scope_condition = "1 = 1"
    series_scope_filter = ""
    requested_series = []
else:
    requested_series = [item.strip().upper() for item in SERIES_IDS_PARAM.split(",") if item.strip()]
    if not requested_series:
        raise ValueError("No series selected for Silver incremental processing.")
    series_values = ", ".join(sql_literal(series_id) for series_id in requested_series)
    series_scope_condition = f"bronze.series_id IN ({series_values})"
    series_scope_filter = f"WHERE series_id IN ({series_values})"

if REQUESTED_BRONZE_RUN_ID:
    PREVIOUS_SILVER_WATERMARK_AT_UTC = None
    change_predicate = (
        f"(bronze.first_seen_bronze_run_id = {sql_literal(REQUESTED_BRONZE_RUN_ID)} "
        f"OR NOT EXISTS ("
        f"SELECT 1 FROM {silver_table('fred_observation_versions_cleaned')} cleaned "
        f"WHERE cleaned.observation_version_id = bronze.observation_version_id"
        f"))"
    )
    change_mode = "requested_bronze_run_id"
else:
    PREVIOUS_SILVER_WATERMARK_AT_UTC = None
    if SERIES_IDS_PARAM.upper() == "ALL" and table_exists(f"{CATALOG}.{SILVER_SCHEMA}.fred_version_run_summary"):
        try:
            watermark_rows = spark.sql(
                f"""
                SELECT MAX(source_max_last_collected_at_utc) AS previous_silver_watermark_at_utc
                FROM {silver_table('fred_version_run_summary')}
                WHERE source_max_last_collected_at_utc IS NOT NULL
                """
            ).collect()
            PREVIOUS_SILVER_WATERMARK_AT_UTC = watermark_rows[0]["previous_silver_watermark_at_utc"] if watermark_rows else None
        except Exception:
            PREVIOUS_SILVER_WATERMARK_AT_UTC = None

    if not PREVIOUS_SILVER_WATERMARK_AT_UTC:
        watermark_rows = spark.sql(
            f"""
            SELECT MAX(last_collected_at_utc) AS previous_silver_watermark_at_utc
    ...
    if not PREVIOUS_SILVER_WATERMARK_AT_UTC:
        raise ValueError(
            "No previous Silver watermark was found for the selected scope. "
            "Run 02a_silver_fred_bootstrap_versions.py first, or pass bronze_run_id explicitly."
        )

    if INCLUDE_MISSING_IN_SILVER:
        missing_not_exists_eligibility = "1 = 1"
    else:
        missing_not_exists_eligibility = (
            "bronze.value_raw IS NOT NULL "
            "AND trim(bronze.value_raw) NOT IN ('', '.') "
            "AND try_cast(trim(bronze.value_raw) AS DOUBLE) IS NOT NULL"
        )

    change_predicate = (
        f"(bronze.first_collected_at_utc > {sql_literal(PREVIOUS_SILVER_WATERMARK_AT_UTC)} "
        f"OR (({missing_not_exists_eligibility}) AND NOT EXISTS ("
        f"SELECT 1 FROM {silver_table('fred_observation_versions_cleaned')} cleaned "
        f"WHERE cleaned.observation_version_id = bronze.observation_version_id"
        f")))"
    )
    change_mode = "since_last_silver_watermark"

spark.sql(
    f"""
    CREATE OR REPLACE TEMP VIEW changed_bronze_versions AS
    SELECT *
    FROM {bronze_table("fred_observation_versions")} bronze
    WHERE {series_scope_condition}
      AND ({change_predicate})
    """
)

changed_bronze_row_count = spark.sql("SELECT COUNT(*) AS row_count FROM changed_bronze_versions").collect()[0]["row_count"]

spark.sql(
    """
    CREATE OR REPLACE TEMP VIEW affected_silver_series AS
    SELECT DISTINCT series_id
    FROM changed_bronze_versions
    WHERE series_id IS NOT NULL
    """
)

affected_series = [row["series_id"] for row in spark.sql("SELECT series_id FROM affected_silver_series ORDER BY series_id").collect()]
affected_series_count = len(affected_series)
selected_series = affected_series

print(f"Incremental change mode: {change_mode}")
print(f"Requested bronze_run_id: {REQUESTED_BRONZE_RUN_ID}")
print(f"Previous Silver watermark: {PREVIOUS_SILVER_WATERMARK_AT_UTC}")
print(f"Changed Bronze observation version rows: {changed_bronze_row_count}")
print(f"Affected series count: {affected_series_count}")
print("Affected series:", ", ".join(affected_series[:20]) + (" ..." if len(affected_series) > 20 else ""))
...
spark.sql(
    ...
    SELECT bronze.*
    FROM {bronze_table("fred_observation_versions")} bronze
    INNER JOIN affected_silver_series affected USING (series_id)
    """
)

selected_count = spark.sql("SELECT COUNT(*) AS row_count FROM selected_bronze_versions").collect()[0]["row_count"]
print(f"Selected Bronze rows for affected-series re-cleaning: {selected_count}")

if affected_series_count == 0:
    dbutils.notebook.exit("No new Bronze observation versions found for Silver incremental cleaning.")
...
spark.sql(
    ...
    SELECT
        ...
        {sql_literal(REQUESTED_BRONZE_RUN_ID)} AS requested_bronze_run_id,
        {sql_literal(PREVIOUS_SILVER_WATERMARK_AT_UTC)} AS previous_silver_watermark_at_utc,
        CAST({changed_bronze_row_count} AS BIGINT) AS changed_bronze_row_count,
        CAST({affected_series_count} AS BIGINT) AS affected_series_count,
